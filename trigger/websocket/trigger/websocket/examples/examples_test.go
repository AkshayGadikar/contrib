...
func TestIntegrationJSON(t *testing.T) {
	...
	for i := range parameters {
		...
		if parameters[i].mode == "1" {
			Input.Trig[0].Handlers[0]["settings"] = map[string]interface{}{"method": "GET", "path": "/ws",
				"mode": parameters[i].mode}
		}
		if parameters[i].mode == "2"{
			Input.Trig[0].Handlers[0]["settings"] = map[string]interface{}{"method": "GET", "path": "/ws", "mode": parameters[i].mode}
			Input.Resources[0].Data.Services[0]["settings"] = map[string]interface{}{ "uri": "ws://localhost:8080/ws",
